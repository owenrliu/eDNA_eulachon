...
There are two versions of the krill data we will use. The first is the raw NASC backscatter data, which we will summarize in various ways to create candidate covariates. The second is a version of the data filtered through kernel density estimation, as in Phillips et al. (2021).
...
kdf1 <- read_csv(here('data','krill', 'HakeSurvey_KrillNASC_0.5nmix10m_cells_2019-2023.csv')) %>%
  filter(Layer_depth_min>140)

kdf2 <- read_csv(here('data','krill','HakeSurvey_KrillNASC_0.5nmix10m_14-150m_2019-2023_AllCombine.csv'))
...
```{r}
kde <- read_csv(here('data','krill','KrillKDE_2019_2023.csv'))
```

Import the processed and cleaned eulachon samples
...
The KDE is flat (depth-integrated), but we can still look at krill distribution by latitude and bottom depth. This differs from the NASC latitude-by-depth plot above, since we use bottom depth here instead of sample depth.
...
edna_krill_matched_5nn <- d_obs %>%
  ...
  ungroup()
toc()
#Note: this join takes ~70 minutes
```
...
As described above, we calculate multiple candidate krill variables. Each has a slightly different way of summarizing or averaging krill values across depth and/or space. We do variable selection later, in the main eulachon SDMs workflow.
...
```{r,eval=F}
# matched to samples
write_rds(edna_krill_metrics,here('data','krill','edna_krill_metrics.rds'))
write_rds(grid.pred.k,here('data','krill','krill_k2_k4_k5_matched_to_pred_grid.rds'))
```
