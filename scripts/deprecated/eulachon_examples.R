...
d_obs_filt <- d_obs_filt %>% mutate(depth_fct=as.factor(depth_cat))
fitfct <- sdmTMB( # AIC 13391.9
  Ct ~  0 + (1 | depth_fct),
  data = d_obs_filt, 
  mesh = mesh_filt,
  # time="year",
...
tidy(fitfct,"ran_pars") # range 60.9km +- 11.4km
tidy(fitfct,"ran_vals") # random intercepts by depth
...
fitfct.pred <- predict(fitfct,newdata=grid.predz) %>% 
  mutate(utm.lon.m=utm.lon.km*1000,
         utm.lat.m=utm.lat.km*1000)
...
ggplot(fitfct.pred,aes(est))+geom_density()
fivenum(exp(fitfct.pred$est))  

# # Map of predictions (fit9)
pred.map <- ggplot()+
...
# Map of predictions (fitfct)
pred.map2 <- ggplot()+
  geom_path(data=bdf,aes(x,y,group=Group),color='gray20')+
  geom_raster(data=fitfct.pred,aes(utm.lon.m,utm.lat.m,fill=est),alpha=0.8)+
  geom_sf(data=coast,fill="gray50")+
  scale_fill_viridis(option='turbo')+
  labs(x="",y="",fill="log(eDNA)")+
  facet_wrap(~depth_cat)+
...
# facet_grid(year~depth_cat)
# pred.map
pred.map2.zoom <- pred.map2+ylim(1e6,2e6)+xlim(7e5,10e5)
pred.map2.zoom
