...
mconds_k4 <- make_cond_plot(m,k4_ln,exp_var=T,saveplot=F)
...
ggsave(here('plots','krill_metric2_by_year_map.png'),k4_plot,w=6,h=10)
```
...
```{r}
#depth-depth plot?
depth_depth_p <- m.preds %>% 
  filter(bathy.bottom.depth<1500) %>% 
  ggplot(aes(bathy.bottom.depth,exp(est),color=factor(year)))+
  geom_point()+facet_wrap(~depth_cat)
depth_depth_p
```
