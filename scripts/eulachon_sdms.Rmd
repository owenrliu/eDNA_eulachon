...
#plot
vs2_p <- tibble(model_number=1:7,vars=kvars_combs,f1=f1_kvar_ll,f3=f3_kvar_ll,f4=f4_kvar_ll) %>% 
  pivot_longer(f1:f4,names_to="model_type",values_to = "ll") %>% 
  ggplot(aes(model_number,ll,color=model_type))+
  geom_point(size=3)+
...
These actually improve our cross-validation likelihood values, particularly variables k2 and k3. Now, the highest-likelihood model is Model 4, with `k2_ln` and `thetao_ln` as predictors.
...
tic("Fitting:")
m <- sdmTMB(
  Ct ~ 1+s(thetao_ln,k=3)+s(k2_ln,k=3)+washed+(1|depth_cat)+(1|yr_fct),
  data = d_obs_filt,
  mesh = mesh,
  offset= "offsets_all",
  spatial="on",
...
mconds_k2 <- make_cond_plot(m,k2_ln,exp_var=T,saveplot=F)

mod1_covar_cond_effs <- cowplot::plot_grid(mconds_thetao,mconds_k2,nrow=2)
ggsave(here('plots','Model 1 Conditional Covariate Effects.png'),mod1_covar_cond_effs,w=4,h=6,bg='white')
```
...
k2_plot <- make_map(grid.pred,k2_ln)+
  scale_fill_viridis(option="C")+
  labs(title="Krill Metric 2 (log)")
thetao_plot
k2_plot

ggsave(here('plots','temperature_by_depth_year_map.png'),thetao_plot,w=6,h=10)
ggsave(here('plots','krill_metric2_by_depth_year_map.png'),k2_plot,w=6,h=10)
```
...
#   mutate(quantcut=cut(quantest,20)) %>% 
#   make_map(quantcut)+
#   ylim(wobbox[c(2,4)])+xlim(wobbox[c(1,3)])+
...
m.preds %>% 
  ...
  scale_fill_viridis(option='D',discrete=F)+
    # scale_fill_manual(breaks=seq(0,1,by=0.1),values=viridis_pal(option="C")(11))+
  labs(title="Model 1",fill="Quantile (eDNA)")
ggsave(here('plots','Model 1 Quantiles WA and OR.png'),w=6,h=8,bg="white")

#presence/absence
m.pa.map <- make_presence_absence_map(m)[[2]]+labs(x="",y="")+
  ylim(wobbox[c(2,4)])+xlim(wobbox[c(1,3)])
ggsave(here('plots','Model 1 presence absence.png'),m.pa.map,w=6,h=5,bg="white")
```
...
calc_mesh_ll <- function(testmesh){
  ...
  fit <- try(sdmTMB_cv(
    # Ct ~ 1+thetao_ln + I(thetao_ln^2)+washed,
    # Ct ~ 1+s(thetao_ln,k=3)+washed,
...
```{r}
# try simpler meshes
meshestest <- map(c(seq(2,10,by=2),seq(20,180,by=20)),function(cuts) make_mesh(d_obs_filt,xy_cols=c('utm.lon.km','utm.lat.km'),type='cutoff',cutoff=cuts))
...
```{r}
test_cuts <- c(seq(2,10,by=2),seq(20,180,by=20))
mesh.comparison <- tibble(cutoff=test_cuts,sumll=c(meshes.ll.test)) %>% 
  ggplot(aes(cutoff,sumll))+
  geom_line()+
  geom_point()+
  # stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1,se=F)+
...
mesh.comparison2 <- tibble(cutoff=test_cuts,sumll=meshes.ll.test) %>% 
  mutate(nverts=map_dbl(meshestest,~.$mesh$n)) %>% 
  ggplot(aes(nverts,sumll))+
  geom_line()+
  geom_point()+
...
meshes_table <- tibble(cutoffs_to_try=test_cuts,sumll=meshes.ll.test) %>% 
  mutate(nverts=map_dbl(meshestest,~.$mesh$n))
...
```{r}
#trying priors on standard curves
# k <- tibble(par=names(f4$sd_report$value),val=f4$sd_report$value)
ftest <- sdmTMB(
  Ct ~ 1+thetao_ln + I(thetao_ln^2)+washed,
  data = d_obs_filt,
  mesh = meshestest[[6]],
...
make_cond_plot(ftest,thetao_ln)
```
...
test<- m.preds %>% 
  arrange(est) %>% 
  mutate(quantest=percent_rank(est)) %>% 
  make_map(quantest)+
  ylim(wobbox[c(2,4)])+xlim(wobbox[c(1,3)])+
  scale_fill_viridis(option='D',discrete=F)+
    # scale_fill_manual(breaks=seq(0,1,by=0.1),values=viridis_pal(option="C")(11))+
